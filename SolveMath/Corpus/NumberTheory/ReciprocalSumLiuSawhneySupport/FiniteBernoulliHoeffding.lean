...
public import SolveMath.Corpus.NumberTheory.ReciprocalSumLiuSawhneySupport.FiniteWeightedSubsetFourier
public import SolveMath.Corpus.NumberTheory.ReciprocalSumLiuSawhneySupport.SmoothPrimePowerFactorization
public import SolveMath.Corpus.NumberTheory.ReciprocalSumLiuSawhneySupport.PrimePowerLCMTelescope

@[expose] public section


namespace FiniteBernoulliHoeffding
...
This file proves the concentration estimate needed in the Liu--Sawhney
lower-bound argument for unit-fraction density without introducing a
probability space.  A product Bernoulli law is simply a nonnegative weight on
`I.powerset`, and all expectations are finite sums.
...
def eventMass (I : Finset ι) (p : ι → ℝ) (E : Finset ι → Prop) : ℝ :=
  ∑ B ∈ I.powerset, if E B then
    FiniteWeightedSubsetFourier.subsetWeight I p B else 0
...
lemma subsetWeight_mul_exp_centered {I B : Finset ι} {p x : ι → ℝ}
    (hB : B ⊆ I) (z : ℝ) :
    FiniteWeightedSubsetFourier.subsetWeight I p B *
        Real.exp (z * centeredSubsetSum I p x B) =
      (∏ i ∈ B, p i * Real.exp (z * ((1 - p i) * x i))) *
        ∏ i ∈ I \ B, (1 - p i) * Real.exp (z * (-p i * x i)) := by
  rw [FiniteWeightedSubsetFourier.subsetWeight, centeredSubsetSum, Finset.sum_ite]
  have hfilter : I.filter (fun i => i ∈ B) = B := by
    ext i
    simp [hB]
  have hfilterc : I.filter (fun i => ¬i ∈ B) = I \ B := by
...
theorem upperTail_le_exp (I : Finset ι) (p x : ι → ℝ)
  ...
  calc
    eventMass I p (fun B => t ≤ centeredSubsetSum I p x B) ≤
      ...
      intro B hB
      have hw := FiniteWeightedSubsetFourier.subsetWeight_nonneg
        I p hp0 hp1 hB
...
theorem abs_centeredSubsetSum_tail (I : Finset ι) (p x : ι → ℝ)
  ...
  · have hmass : eventMass I p
      ...
      calc
        eventMass I p (fun B => t ≤ |centeredSubsetSum I p x B|) ≤
            ∑ B ∈ I.powerset,
              FiniteWeightedSubsetFourier.subsetWeight I p B := by
          rw [eventMass]
          apply Finset.sum_le_sum
          intro B hB
          split_ifs
          · exact le_rfl
          · exact FiniteWeightedSubsetFourier.subsetWeight_nonneg
              I p hp0 hp1 hB
        _ = 1 := FiniteWeightedSubsetFourier.sum_subsetWeight I p
    rw [← hzero]
    norm_num
...
in the prime-power LCM development. -/
theorem eventually_smoothLcm_le_exp_five_mul :
    ∀ᶠ S : ℕ in atTop,
      (SmoothPrimePowerFactorization.smoothLcm S : ℝ) ≤
        Real.exp (5 * (S : ℝ)) := by
  filter_upwards
    [PrimePowerLCMTelescope.eventually_initialLcm_le_exp_two_mul]
      with S hS
...
theorem abs_reciprocal_sum_sub_mean_tail_le_inv_four_smoothLcm
    ...
    (hQ : (SmoothPrimePowerFactorization.smoothLcm S : ℝ) ≤
      Real.exp (5 * (S : ℝ))) :
    eventMass I p (fun B =>
        1 ≤ |subsetSum B (fun n : ℕ => ((n : ℝ)⁻¹)) -
          subsetMean I p (fun n : ℕ => ((n : ℝ)⁻¹))|) ≤
      1 / (4 *
        (SmoothPrimePowerFactorization.smoothLcm S : ℝ)) := by
  have hN : 0 < N := lt_of_lt_of_le hM hMN
  have hNreal : 0 < (N : ℝ) := by exact_mod_cast hN
  ...
  calc
    ...
    _ ≤ 1 / (4 *
      ...
      calc
        2 * Real.exp (-12 * (S : ℝ)) *
              (4 * (SmoothPrimePowerFactorization.smoothLcm S : ℝ)) =
            8 * (SmoothPrimePowerFactorization.smoothLcm S : ℝ) *

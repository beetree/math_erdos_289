...
public import SolveMath.Corpus.NumberTheory.UnitFractionSubsumResults.TechnicalProposition
...
/-- Ported from `SolveMath.Corpus.Analysis.UnitFractionAnalyticMisc.Finset.Icc_sdiff_Icc_left`,
which is not in the kept-module set; restated locally since it is used below. -/
...
lemma count_multiples'' {m n : ℕ} (hm : 1 ≤ m) :
  ...
  rw [UnitFractions.count_multiples hm, Int.self_sub_fract, ← natCast_floor_eq_intCast_floor,
    Nat.floor_div_eq_div]
  exact div_nonneg (Nat.cast_nonneg _) (Nat.cast_nonneg _)

lemma count_multiples''' {m n : ℕ} (hm : 1 ≤ m) :
    (((Icc 1 n).filter fun k => m ∣ k).card : ℝ) ≤ (n / m : ℝ) := by
  rw [UnitFractions.count_multiples hm]
...
lemma sum_prime_counting_sq :
  ...
  have hsplit :
    ...
    rw [← Finset.sum_filter_add_sum_filter_not ((Icc 1 N).filter Nat.Prime) (fun q => p = q),
      Finset.sum_filter, Finset.sum_ite_eq, ite_eq_left hp]
    ...
    · intro q hq
      ...
      · rw [ite_eq_left h]
        ...
        rw [ite_eq_left (hpqcop.mul_dvd_of_dvd_of_dvd h.1 h.2)]
      · rw [ite_eq_right h]
        split_ifs
        · exact zero_le_one
        · rfl
...
lemma count_divisors' {x N : ℕ} (hx : x ≠ 0) (hN : N ≠ 0) :
  ...
  rw [range_eq_insert_Icc hN', Finset.filter_insert, ite_eq_left (dvd_zero x), card_insert_of_notMem,
  ...
  · let P :=
    ...
    have h₁ :
      ...
      · rintro ⟨hn, hn'⟩
        ...
        rw [Nat.coprime_prod_right_iff] at hn'

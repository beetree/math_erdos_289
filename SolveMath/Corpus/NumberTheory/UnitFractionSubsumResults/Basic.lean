...
lemma explicit_mertens2 :
    ∀ᶠ N : ℕ in atTop,
      (((Finset.range (N + 1)).filter IsPrimePow).sum (fun q ↦ (1 / q : ℝ)) : ℝ) ≤
        (501 / 500 : ℝ) * log (log N) :=
  explicit_mertens_ge (K := 501 / 500) (by norm_num)
...
lemma force_good_properties_hIcard0
...
lemma force_good_properties_hIcardn0
    (N : ℕ) (M t : ℝ) (I : Finset ℤ)
    (hI :
      I =
        Finset.Icc
...
lemma force_good_properties_hIcard'
    (N : ℕ) (M t : ℝ) (I : Finset ℤ)
    (hIcard0 :
      (I.card : ℤ) =
        ⌊t + (M * (N : ℝ) ^ (-(2 : ℝ) / log (log N))) / 2⌋ + 1 -
...
lemma force_good_properties_hlarge9
    (N : ℕ) (M : ℝ) (I : Finset ℤ)
    (hlarge : 1 < N)
    (h0M : 0 < M)
    (hlargeNs :
...
lemma force_good_properties_hIclose'
    (N : ℕ) (M t : ℝ) (I : Finset ℤ)
    (hI :
      I =
        Finset.Icc
...
lemma force_good_properties_hIclose
  ...
  rw [show (|(x : ℝ) - y| : ℝ) = ((Int.natAbs (x - y) : ℕ) : ℝ) by
    calc
      |(x : ℝ) - y| = |((x - y : ℤ) : ℝ)| := by norm_num
      _ = ((Int.natAbs (x - y) : ℕ) : ℝ) := by
        rw [← Int.cast_abs, Int.abs_eq_natAbs, Int.cast_natCast]] at hxy
  exact_mod_cast hxy

...
-/
import SolveMath.Edges.Erdos.P300.Statement
import SolveMath.Corpus.NumberTheory.ReciprocalSumLiuSawhneySupport.FiniteBernoulliHoeffding
import SolveMath.Corpus.NumberTheory.ReciprocalSumLiuSawhneySupport.FiniteWeightedSubsetFourier
import SolveMath.Corpus.NumberTheory.ReciprocalSumLiuSawhneySupport.SmoothPrimePowerFactorization
import SolveMath.Corpus.NumberTheory.ReciprocalSumLiuSawhneySupport.PrimePowerLCMTelescope
import SolveMath.Corpus.NumberTheory.UnitFractionDensities
import SolveMath.Corpus.Analysis.UnitFractionFourierAnalysis.Basic
import SolveMath.Corpus.Analysis.UnitFractionFourierAnalysis.ExponentialSums
import SolveMath.Corpus.Analysis.UnitFractionFourierAnalysis.MajorArcs
import SolveMath.Corpus.Analysis.UnitFractionFourierAnalysis.CircleMethod
import SolveMath.Corpus.NumberTheory.UnitFractionSubsumResults.Basic
import SolveMath.Corpus.NumberTheory.UnitFractionSubsumResults.TwoValues
import SolveMath.Corpus.NumberTheory.UnitFractionSubsumResults.GoodProperties
import SolveMath.Corpus.NumberTheory.UnitFractionSubsumResults.PruningLemmas
import SolveMath.Corpus.NumberTheory.UnitFractionSubsumResults.LargeEstimates
import SolveMath.Corpus.NumberTheory.UnitFractionSubsumResults.TechnicalProposition
import SolveMath.Corpus.NumberTheory.UnitFractionFinalResults.PrimeCountingSieve
import SolveMath.Corpus.NumberTheory.UnitFractionFinalResults.FilterDiv
import SolveMath.Corpus.NumberTheory.UnitFractionFinalResults.UpperDensity
import SolveMath.Corpus.NumberTheory.UnitFractionAuxiliaryEstimates.Basic
...
open UnitFractions PrimePowerLCMTelescope

-- The FourierW21 module transitively exports a root-level `e : ℝ → BoundedContinuousFunction ℝ ℂ`,
-- so unqualified `e` becomes ambiguous.  Shadow it inside this namespace with the unit-fraction
-- additive character expressed in the normal form the upstream proof expects.
noncomputable def e (x : ℝ) : ℂ := Complex.exp ((x : ℂ) * (2 * Real.pi * Complex.I))
...
private lemma e_zero : e (0 : ℝ) = 1 := by
  simp only [e, Complex.ofReal_zero, zero_mul, Complex.exp_zero]

private lemma e_int (z : ℤ) : e (z : ℝ) = 1 := by
  simp only [e]
  rw [show ((z : ℝ) : ℂ) * (2 * Real.pi * Complex.I) = (z : ℂ) * (2 * Real.pi * Complex.I) by
    simp only [Complex.ofReal_intCast]]
  exact Complex.exp_int_mul_two_pi_mul_I z

private lemma norm_e (x : ℝ) : ‖e x‖ = 1 := by
  simp [e, Complex.norm_exp]

private lemma e_sum {ι : Type*} {s : Finset ι} {f : ι → ℝ} :
    e (s.sum f) = s.prod (fun i => e (f i)) := by
  classical
  induction s using Finset.induction_on with
  | empty => simp [e_zero]
  | @insert a s ha ih =>
      simp [Finset.sum_insert ha, Finset.prod_insert ha, e_add, ih]

private lemma UnitFractions_e_eq_e (x : ℝ) : UnitFractions.e x = e x := by
  simp only [e, UnitFractions.e, AdditiveCharacterGeometricSums.additiveCharacter]
  congr 1
  ring_nf

private lemma card_valid_sum_range (t : ℕ) : (valid_sum_range t).card = t := by
  rw [valid_sum_range]
  have h2 : ((t : ℤ) / 2 : ℤ) - (-(t : ℤ) / 2 : ℤ) = (t : ℤ) := dumb_subtraction_thing t
  rw [Int.card_Ioc (-(t : ℤ) / 2) ((t : ℤ) / 2)]
  rw [h2]
  norm_cast
...
private lemma subsetWeight_eq_finiteHoeffding
    ...
    subsetWeight A τ B =
      FiniteWeightedSubsetFourier.subsetWeight A (fun _ => τ) B := by
  simp [subsetWeight, FiniteWeightedSubsetFourier.subsetWeight,
...
private lemma character_sum_eq_lcm_indicator
  ...
  simp only [UnitFractions_e_eq_e] at horth
  have hphase : ∀ h : ℤ,
      e ((h : ℝ) * (rec_sum B : ℝ)) =
        e ((h : ℝ) * (u : ℝ) / (Q : ℝ)) := by
    intro h
...
private lemma integerReciprocalMass_le_hoeffdingTail_of_avoids
    ...
    integerReciprocalMass A τ ≤
      FiniteBernoulliHoeffding.eventMass A (fun _ => τ) (fun B =>
        1 ≤ |FiniteBernoulliHoeffding.subsetSum B
              (fun n : ℕ => (n : ℝ)⁻¹) -
            FiniteBernoulliHoeffding.subsetMean A (fun _ => τ)
              (fun n : ℕ => (n : ℝ)⁻¹)|) := by
  classical
  have hmeanFH :
      FiniteBernoulliHoeffding.subsetMean A (fun _ => τ)
          (fun n : ℕ => (n : ℝ)⁻¹) = 1 := by
    rw [FiniteBernoulliHoeffding.subsetMean, ← Finset.mul_sum]
    calc
      τ * ∑ i ∈ A, (i : ℝ)⁻¹ = τ * (rec_sum A : ℝ) := by
        congr 1
        simpa [one_div] using (rec_sum_cast_real A).symm
      _ = 1 := hmean
  rw [integerReciprocalMass, FiniteBernoulliHoeffding.eventMass]
  apply Finset.sum_le_sum
  intro B hB
  have hBA : B ⊆ A := Finset.mem_powerset.mp hB
  have hweight0 : 0 ≤ subsetWeight A τ B := by
    rw [subsetWeight_eq_finiteHoeffding hBA]
    exact FiniteWeightedSubsetFourier.subsetWeight_nonneg A (fun _ => τ)
      (fun _ _ => hτ0) (fun _ _ => hτ1) hB
  have hweight0FH :
      0 ≤ FiniteWeightedSubsetFourier.subsetWeight A (fun _ => τ) B := by
  ...
  · obtain ⟨d, hd⟩ := hdiv
    ...
    have hfarD : (1 : ℝ) ≤ |(d : ℝ) - 1| := by
      ...
      · have hdTwo : 2 ≤ d := by omega
        ...
        · linarith
    have hsubset :
        FiniteBernoulliHoeffding.subsetSum B
            (fun n : ℕ => (n : ℝ)⁻¹) = (d : ℝ) := by
...
private lemma weighted_fourier_real_lower
  ...
  have hzeroProd : (bernoulliProd A τ 0).re = 1 := by
    simp [bernoulliProd, bernoulliFactor, e_zero]
  have hvalidSplit :
...
private lemma top_interval_minimizes_rec_sum {N : ℕ} {A : Finset ℕ}
  ...
  | h N ih =>
      ...
      have hrecA : (rec_sum A : ℝ) =
        ...
        simp only [rec_sum]
        rw [Finset.sum_insert (by simp [A'])]
        push_cast
        ring
      rw [hcardA'] at hrec'
      rw [htop, hrecA]
...
private lemma eventually_scaled_hoeffding_tail {eta : ℝ}
  ...
  have htail := FiniteBernoulliHoeffding.abs_reciprocal_sum_sub_mean_tail
    ...
    have hevent0 : 0 ≤
        FiniteBernoulliHoeffding.eventMass A (fun _ => τ) (fun B =>
          1 ≤ |FiniteBernoulliHoeffding.subsetSum B
              ...
              FiniteBernoulliHoeffding.subsetMean A (fun _ => τ)
                (fun n : ℕ => (n : ℝ)⁻¹)|) := by
      rw [FiniteBernoulliHoeffding.eventMass]
      apply Finset.sum_nonneg
      intro B hB
      split_ifs
      · exact FiniteWeightedSubsetFourier.subsetWeight_nonneg A

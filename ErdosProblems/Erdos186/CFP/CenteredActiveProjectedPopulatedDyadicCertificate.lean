...
import ErdosProblems.Erdos186.CFP.CenteredScaledActiveProjectedPhysicalDensityTargetCertificate
...
/-!
# Populated dyadic projected certificate in active centered coordinates

This is the projected-properization version of the populated dyadic join.
The random colouring, varying dyadic runs, and active-coordinate span
transport are unchanged.  Projection removes the old block and no-carry
premises and returns a source-line `FixedScaleWitness` directly.
-/
...
/-- Structural source-facing join using active centered coordinates and
projected properization.  The output denominator is uniform over all active
ranks at most `D`. -/
theorem exists_centeredActiveProjectedPopulatedDyadicCertificateConstants
  ...
  have hconstants : ∀ i : Fin (D + 1),
    ...
    · obtain ⟨corWidth, denseConstant, denseEll, denseWidth,
          hdense, hcert⟩ :=
        exists_centeredScaledActiveProjectedPhysicalDensityTargetCertificateConstants
      ...
      intro _hpos W B A P hPproper hPnondegenerate hBW hAB hzeroW hzeroB
        ...
        hblock hdD
        hambient hsourceBound htarget hreserveLower hsUpper hscaleUpper
        hprojection
      exact hcert P hPproper hPnondegenerate hBW hAB hzeroW hzeroB
        (D := D) (block := block) c run hell hwidth hCell hsourcePos hblock hdD hambient
        hsourceBound htarget hreserveLower hsUpper hscaleUpper hprojection
    · refine ⟨1, 1, 1, 1, by omega, ?_⟩
      intro hpos
      omega
  choose corWidth denseConstant denseEll denseWidth hspec using hconstants
  ...
  have hdenseWidthLe (i : Fin (D + 1)) : denseWidth i ≤ denseWidthMax := by
    exact (Finset.single_le_sum (fun _ _ ↦ Nat.zero_le _)
      (Finset.mem_univ i)).trans (Nat.le_add_left _ 1)
  have hdenseMax : 0 < denseMax := by simp [denseMax]
  ...
  intro source W B relevant d hproper hdrel hWsource hBW hzeroW hzeroB
    ...
    hblock hell hwidth hCell hcapSource hroom hsFold hprojectionUniform
  ...
  have htargetRaw :
    ...
    have hsourceFold : sourceScale ≤ fold := hsFold
    have hvolume :=
  ...
  have hcertificate := hactual.2 hactiveIndex
    (W := W) (B := B) (A := A)
  have htargetActive :
  ...
  obtain ⟨hsourcePos, _hblockedLower, hsUpper, hreserveLower⟩ :=
  ...
  have hprojectionActual : ProjectedProperization.projectionFactor D ≤
      sourceScale * ((q + 1) / denseConstant activeIndex) := by
    apply hprojectionUniform.trans
    exact Nat.mul_le_mul_left sourceScale
      (Nat.div_le_div_left (hdenseLe activeIndex) hactual.1)
  obtain ⟨k, ⟨C⟩⟩ := hcertificate Pactive
    (Pfull.activeDimensions_proper (hproper.proper hdrel))
    Pfull.activeDimensions_nondegenerate hBW (Finset.erase_subset 0 B)
    hzeroW hzeroB c run hellActual hwidthActual hCellActual
    hsourcePos hblock hactiveD hspanActive hrunSource htargetActive
    hreserveLower hsUpper hscaleUpper hprojectionActual
  have hdenLe :
      (4 * denseConstant activeIndex * block) *
          ProjectedProperization.projectionFactor D ≤
        (4 * denseMax * block) *
          ProjectedProperization.projectionFactor D :=
    Nat.mul_le_mul_right _
      (Nat.mul_le_mul_right block
        (Nat.mul_le_mul_left 4 (hdenseLe activeIndex)))
  refine ⟨k, ⟨C.increaseScaleDen hdenLe ?_⟩⟩
  exact Nat.mul_pos (Nat.mul_pos (Nat.mul_pos (by omega) hdenseMax) hblock)

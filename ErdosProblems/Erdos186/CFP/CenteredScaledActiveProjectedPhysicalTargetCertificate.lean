...
Authors: Codex
-/
import ErdosProblems.Erdos186.CFP.CenteredScaledActiveProjectedPhysicalReserveCertificate
import ErdosProblems.Erdos186.CFP.CenteredScaledActivePhysicalTargetCertificate

/-! # Physical target completion before projected properization -/
...
/-- Generator completion for varying physical runs, followed by the
coordinate-first projected-properization sink. -/
theorem exists_centeredScaledActiveProjectedPhysicalTargetCertificateConstants
    ...
    ∃ corWidth denseConstant denseEll denseWidth : ℕ,
      ...
      ∀ {W B A : Finset ℤ}
        ...
        ∀ {q cap target sourceScale s D K block : ℕ}
          (c : {a // a ∈ A} → Fin (q + 1))
          ...
          (∀ i,
            (Stability.generatedSubgroup
                ...
                (Greedy.selected (integerColorClass A c i)
                  (run i).steps)).relIndex
              (Stability.generatedSubgroup
                (Preprocessing.centeredIdentification P hPproper hzeroW)
  ...
  obtain ⟨corWidth, denseConstant, denseEll, denseWidth,
    ...
    exists_centeredScaledActiveProjectedPhysicalReserveCertificateConstants
  ...
  intro W B A P hPproper hPnondegenerate hBW hAB hzeroW hzeroB q cap
    target sourceScale s D K block c run hell hwidthScale hCell hsourceScale hblock hdD
    hfinite hindex hambient hsourceBound htarget hreserveLower hsUpper
    hscaleUpper hprojection
  ...
  have hreserveDisjoint : (Set.univ : Set (Fin (q + 1))).PairwiseDisjoint
    ...
    have hab : a = b := Stability.integerPoint_injective (hax.trans hbx.symm)
    subst b
  ...
  have hreserveSmall :
    ...
    calc
      ...
      _ ≤ s := hreserveLower
  exact hcertificate P hPproper hPnondegenerate hBW hzeroW hzeroB
    selected completed hell hwidthScale hCell hsourceScale hblock hdD hselected
    hcompletedCard hcompletedSubset hphysicalDensity hgenerated
    hreserveDisjoint hreserveSmall hsUpper hscaleUpper hprojection

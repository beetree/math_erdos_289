...
/-!
# Physical density supplies the active-coordinate completion index
-/
...
/-- The active-coordinate varying-run certificate with the completion index
deduced from the same physical density estimate. -/
theorem exists_centeredScaledActivePhysicalDensityTargetCertificateConstants
  ...
  obtain ⟨corConstant, corWidth, denseConstant, denseEll, denseWidth,
    ...
    exists_centeredScaledActivePhysicalTargetCertificateConstants
  ...
  intro W B A P hPproper hPnondegenerate hBW hAB hzeroW hzeroB q cap
    target sourceScale s D block c run hell hwidthScale hCell hsourceScale
    hblock hdD hambient hsourceBound htarget hreserveLower hsUpper

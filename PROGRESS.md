...
Last updated: 2026-09-03 22:50 PDT (commit after `1e15155`). Live view: `scripts/status.sh`.
...
|---|---|---|---|---|---|---|
| S1 | `equidist_inverse` | `Lemma1EquidistStmt.lean` | §2, lines 135–148 | Inverses `t⁻¹ mod U` for `t` in a prefix, `U ∈ {q, 2q, 4q, 4pq}`, are equidistributed in residue intervals with error `o(q^ε)`, from Bourgain–Garaev + Erdős–Turán | Opus (`equidist`) writing `Lemma1Equidist.lean`; Gemini competitors `equidistB`, `equidistC` writing variant files; first to finish gets swapped in | 1 (Sonnet, partial) + 3 running |
...
|---|---|---|---|---|
| `equidist` | `claude-johan`, Opus, high | `Lemma1Equidist.lean` (new) | S1 | running |
| `equidistB` | `agy`, Gemini 3.8 Flash high | `Lemma1EquidistB.lean` (new) | S1, competitor, strategy B | running |
